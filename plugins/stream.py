...
async def channel_receive_handler(c: Client, m: Message bot, broadcast ):
    ...
    try:
        ...
        shortened_link = await get_channel_shortlink(m.chat.id, stream_link)

        await log_msg.reply_text(
            text=f"<b>Channel Name :- {m.chat.title}\nChannel ID :- <code>{m.chat.id}</code>\nRequest URL :- {stream_link}\nShortener Link :- {shortened_link}</b>",
            quote=True,
            parse_mode=ParseMode.HTML
        )
        ...
        await c.send_message(chat_id=FILES_CHANNEL,
                             text=f"<b>Got FloodWait of {str(w.x)}s From {m.chat.title}\n\nChannel ID :-</b> <code>{str(m.chat.id)}</code>",
                             disable_web_page_preview=True, parse_mode=ParseMode.HTML)
    except Exception as e:
        await c.send_message(chat_id=FILES_CHANNEL, text=f"<b>#Error_Trackback :-</b> <code>{e}</code>", disable_web_page_preview=True, parse_mode=ParseMode.HTML)
